...
public final class PackageChunk extends ChunkWithChunks {
    ...
    public void addType(TypeChunk typeChunk) {
        types.put(types.size(), typeChunk);
        if (!getChunks().containsValue(typeChunk))
            addChunk(typeChunk);
    }

    public void addTypeSpecs(TypeSpecChunk typeSpecChunk) {
        typeSpecs.put(typeSpecs.size() + 1, typeSpecChunk);
        if (!getChunks().containsValue(typeSpecChunk))
