...
public final class TypeSpecChunk extends Chunk {
    ...
    public void addResources(int new_resources) {
        int[] ints = new int[resources.length + 1];
        System.arraycopy(resources, 0, ints, 0, resources.length);
        ints[resources.length] = new_resources;
        resources = ints;
    }


    public TypeSpecChunk(Chunk parent, int headerSize, int chunkSize, int offset, int id, int[] resources) {
        super(parent, headerSize, chunkSize, offset);
        this.id = id;
