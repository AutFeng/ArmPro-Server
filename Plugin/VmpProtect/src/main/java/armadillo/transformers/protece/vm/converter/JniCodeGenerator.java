...
public class JniCodeGenerator {
    ...
    private final List<String> WithMethodNames;
    ...
    public JniCodeGenerator(Interpreter interpreter, String dexName, Set<ClassDef> defs, InstructionRewriter instructionRewriter, List<String> keepMethodNames, List<String> withMethodNames) {
        this.interpreter = interpreter;
        this.dexName = dexName;
        this.defs = defs;
        this.instructionRewriter = instructionRewriter;
        this.KeepMethodNames = keepMethodNames;
        this.WithMethodNames = withMethodNames;
    }
    ...
    public void generate(Writer writer) throws IOException {
        ...
        for (ClassDef def : defs) {
            ...
            for (Method method : def.getMethods()) {
                if (WithMethodNames.size() > 0 && !WithMethodNames.contains(method.getName()))
                    continue;
                ...
                if (RegisterNatives.get(key) != null)
                    RegisterNatives.get(key).add(String.format("\t{\"%s\", \"%s\", (void *) %s},\n",
                            MethodHelper.genMethodName(method),
                ...
                switch (interpreter) {
                    ...
                    break;
                    case switch_vm_2: {
                        writer.write(String.format("\tShadowFrame *shadow_frame = ShadowFrame::CreateDeoptimizedFrame(env, %d);\n", method.getImplementation().getRegisterCount()));
                        int preg = method.getImplementation().getRegisterCount() - MethodUtil.getParameterRegisterCount(method);
                        if (!AccessFlags.STATIC.isSet(method.getAccessFlags())) {
                            writer.write(String.format("\tshadow_frame->SetVRegReference(%d ,thiz);\n", preg));
                            preg++;
                        }
                        for (int i = 0; i < method.getParameterTypes().size(); i++) {
                            switch (method.getParameterTypes().get(i).charAt(0)) {
                                case 'D':
                                    writer.write(String.format("\tshadow_frame->SetVRegDouble(%d,p%d);\n", preg, i));
                                    preg += 2;
                                    break;
                                case 'J':
                                    writer.write(String.format("\tshadow_frame->SetVRegLong(%d,p%d);\n", preg, i));
                                    preg += 2;
                                    break;
                                case 'F':
                                    writer.write(String.format("\tshadow_frame->SetVRegFloat(%d,p%d);\n", preg, i));
                                    preg++;
                                    break;
                                case '[':
                                case 'L':
                                    writer.write(String.format("\tshadow_frame->SetVRegReference(%d, p%d);\n", preg, i));
                                    preg++;
                                    break;
                                default:
                                    writer.write(String.format("\tshadow_frame->SetVReg(%d, p%d);\n", preg, i));
                                    preg++;
                                    break;
                            }
                        }
                ...
                switch (interpreter) {
                    case switch_vm_2:
                        writer.write("" +
                                "\tconst VmCode vmCode = {inst, NULL, NULL, tries};\n" +
