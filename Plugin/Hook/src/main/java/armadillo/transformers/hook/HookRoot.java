...
public class HookRoot extends OtherTransformer {
    @Override
    public void transform() throws Exception {
        ...
        contentProviderEditor.setEditorInfo(new ContentProviderEditor.Editorinfo()
                .with(new ContentProviderEditor.ProviderInfo("arm.HideRoot", SHAUtils.SHA1(UUID.randomUUID().toString()), false, 19999999)));
        contentProviderEditor.commit();
        axmlDoc.build(outputStream);
        axmlDoc.release();
        getReplacerRes().put("AndroidManifest.xml", outputStream.toByteArray());
        getAdd_Classdex().add(LoaderRes.getInstance().getStaticResAsBytes("dex/root_hook.dex"));
